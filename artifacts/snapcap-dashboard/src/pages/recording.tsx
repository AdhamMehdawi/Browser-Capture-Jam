...
function generateCypressSpec(events: AnyEvent[], pageUrl: string | null): string {
  ...
  lines.push(`// Generated by SnapCap from a recording`);
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            onClick={() => {
              ...
              a.href = url;
              a.download = `openjam-${recording.id}.cy.js`;
              a.click();
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                      ...
                      {event.type === 'click' && (
                        <div className="truncate">
                          ...
                          <code className="text-foreground text-[11px]">{event.selector || event.message}</code>
                          {event.targetText && (
                            <span className="text-muted-foreground"> — "{event.targetText}"</span>
                          )}
                        </div>
                      )}
                      {event.type === 'input' && (
                        <div className="truncate">
                          ...
                          <code className="text-foreground text-[11px]">{event.selector}</code>
                          <span className="text-muted-foreground"> = </span>
                          <span className="text-emerald-400">{event.value ?? ''}</span>
                        </div>
                      )}
                      {event.type === 'select' && (
                        <div className="truncate">
                      ...
                      {event.type === 'navigation' && (
                        <div className="truncate">
                          <span className="text-muted-foreground">→ </span>
                          <span className="text-sky-400">{event.url}</span>
                        </div>
                      )}
                    </div>
                    {event.duration && (

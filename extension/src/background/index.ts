...
async function showOverlayOn(tabId: number, startedAt: number): Promise<void> {
  try {
    ...
    try {
      ...
      console.warn('[openjam/bg] overlay inject failed', e);
    }
  }
}
...
function setError(code: string, message: string): void {
  ...
  console.error('[openjam/bg]', code, message);
}
...
async function ensureOffscreen(): Promise<void> {
  ...
  if (!existing.length) {
    console.log('[openjam/bg] creating offscreen', OFFSCREEN_URL);
...
async function handleRecordStart(req: RecordStartReq): Promise<BgResponse> {
  console.log('[openjam/bg] record-start', req);
  clearError();
  try {
    if (state.kind !== 'idle') {
    ...
    const blocked = unsupported(tab);
    if (blocked) return blocked;
    console.log('[openjam/bg] target tab', { id: tab.id, url: tab.url });
    ...
    console.log('[openjam/bg] ensuring offscreen');
    await ensureOffscreen();
    console.log('[openjam/bg] sending start to offscreen', {
      ...startPayload,
      mic: req.withMic,
    });
    ...
    console.log('[openjam/bg] offscreen started?', started);
    if (!started?.ok) {
    ...
    console.log('[openjam/bg] state=recording');
...
async function onRecordedFromOffscreen(msg: {
  ...
  try {
    ...
    // Grab context from the active tab (best effort — the user may have
    // navigated, in which case we fall back to a minimal payload).
    ...
    const jam = await api.createJam({
      ...
      visibility: 'PUBLIC',
      media: { kind: 'video', dataUrl: msg.dataUrl },
    });
    resolveResult({
      ok: true,
      id: jam.id,
    ...
    if (e instanceof ApiError) {
      resolveResult({ ok: false, code: e.code, message: e.message });
    } else {
      resolveResult({ ok: false, code: 'upload_failed', message: e instanceof Error ? e.message : String(e) });
    }
  } finally {
    state = { kind: 'idle' };
    await closeOffscreen();
